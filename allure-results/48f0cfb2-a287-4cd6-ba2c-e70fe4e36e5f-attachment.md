...
Locator: getByText('Booking cancelled successfully')
Expected: visible
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 30000ms
  - waiting for getByText('Booking cancelled successfully')
...
- main:
  ...
  - text: "H-CRZ5IB confirmed #116895"
  ...
  - text: "H-8O7JSD confirmed #116894"
  - heading "Hollywood Monsoon Night — Los Angeles" [level=3]
  - text: 📅 12 Jul 2026 🎫 1 ticket 📍 Los Angeles 🗓 Booked 10 Aug 2026
  - paragraph: $2,500
  - paragraph: total
  - link "View Details":
    ...
    - button "View Details"
  - button "Cancel Booking"
  - text: "H-VJS5CJ confirmed #116893"
  - heading "Hollywood Monsoon Night — Los Angeles" [level=3]
  - text: 📅 12 Jul 2026 🎫 1 ticket 📍 Los Angeles 🗓 Booked 10 Aug 2026
  - paragraph: $2,500
  - paragraph: total
...
- dialog "Cancel this booking?":
  ...
  - button "Close":
    - img
  - paragraph: This will cancel H-CRZ5IB and release 1 seat(s) back to the event. This action cannot be undone.
  - button "Cancel"
...
```ts
  ...
  55 | 
  56 | 
  57 |         const bookingCards = this.page.locator('#booking-card');
  58 |         const count = await this.bookingRefs.count();
  59 |         for (let i = 0; i < count; i++) {
  60 |             const card = bookingCards.nth(i);
  61 |             const refText = await this.bookingRefs.nth(i).textContent();
  62 |             if (refText?.trim() === ref) {
  63 |                 Logger.info(`Found booking card for ref: ${ref}`);
  64 | 
  65 |                 await card.locator('[data-testid="cancel-booking-btn"]').click();
  66 |                 this.page.on('dialog', async dialog => {
  67 |                     Logger.info(`Dialog message: ${dialog.message()}`);
