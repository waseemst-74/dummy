...
- dialog "Cancel this booking?":
  ...
  - button "Close":
    - img
  - paragraph: This will cancel H-UQXWDI and release 1 seat(s) back to the event. This action cannot be undone.
  - button "Cancel"
...
> 77 |         await expect(this.page.locator('p:text("Booking cancelled successfully")')).toBeVisible();
     |                                                                                     ^ Error: expect(locator).toBeVisible() failed
  78 | 
  79 |         await expect(remainingRefs).not.toContain(ref);
  80 |         Logger.info(`Booking with ref ${ref} successfully cancelled`);
  81 | 
  82 |     }
  83 | }
```