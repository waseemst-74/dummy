...
- dialog "Cancel this booking?":
  ...
  - button "Close":
    - img
  - paragraph: This will cancel H-8O7JSD and release 1 seat(s) back to the event. This action cannot be undone.
  - button "Cancel"
...
```ts
  ...
  74 |             }
  75 |         }
> 76 |         await expect(this.page.getByText('Booking cancelled successfully')).toBeVisible();
     |                                                                             ^ Error: expect(locator).toBeVisible() failed
  77 | 
  78 |         // After cancellation, re-check all refs
  79 |         const remainingRefs = await this.getAllBookingRefs();
